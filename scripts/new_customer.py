...
'''
    files["portal/customer-config.js"] = portal_config
...
def print_next_steps(info, repo):
  ...
  2. CLOUDFLARE ACCESS (2FA protection)
     Go to: one.dash.cloudflare.com → Access controls → Applications
     ...
     Domain:          {BASE_DOMAIN}
     Session:         24 hours
     Proxy (DNS):     MUST be ON (orange cloud) for this subdomain

     ⚠️  IMPORTANT — Two separate steps in Cloudflare:
     a) Create the APPLICATION (Basic info + hostname above)
     b) Go to the Policies tab inside the app → Add a policy
        Policy name: Allow Client
        Action:      Allow
        Include:     Emails → add client's email address(es)
        → Save policy (policy MUST be assigned inside the app
          or the OTP code will never be sent)
